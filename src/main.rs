...
fn take_screenshot(searched_window_name: String) -> Result<()> {
    unsafe {
        let xlib = x11_dl::xlib::Xlib::open()?;

        let display = (xlib.XOpenDisplay)(std::ptr::null());
        if display.is_null() {
            bail!("Unable to open X server display")
        }
        let root_win = (xlib.XDefaultRootWindow)(display);

        let Some(target_window) = find_window_recursive(&xlib, &searched_window_name, root_win, display)? else {
            bail!("Unable to find the window with title {:?}", searched_window_name)
        };
        let image_buffer = get_window_data(&xlib, target_window, display)?;

        image_buffer.save("test.png")?;

        println!("Finished successfully for {:#x}", target_window);
        return Ok(())
    }
}
...
fn get_window_data(
    ...
    unsafe {
        let mut attributes: x11_dl::xlib::XWindowAttributes = unsafe { std::mem::zeroed() };
        if (xlib.XGetWindowAttributes)(display, window, &mut attributes) == 0 {
            bail!("Unable to get the window attributes of {:#x}", window)
        }
        ...
        if image.is_null() {
            bail!("Unable to get the pxiel data from window {:#x}", window)
        }
...
fn find_window_recursive(
    ...
    window: x11_dl::xlib::Window,
    display: *mut x11_dl::xlib::Display,
) -> Result<Option<x11_dl::xlib::Window>> {
    unsafe {
        ...
        if !(xlib.XQueryTree)(display, window, &mut root_return, &mut parent_return, &mut children, &mut nchildren) == 0 {
            bail!("Unable to query the root window tree for window {:x}", window)
        }
        if children.is_null() {
            return Ok(None)
        }
        ...
        for &child in child_array.iter() {
            let title = get_window_title(&xlib, display, child)?.unwrap_or(String::from("N/A"));
            println!("Discovering window ID: {:#x} with name [{:?}]", child, title);
            if title == *searched_window_name {
                return Ok(Some(child));
            }

            let res = find_window_recursive(xlib, searched_window_name, child, display)?;
            if res.is_some() {
                return Ok(res)
            }
        }
...
fn main() {
    take_screenshot(String::from("~ : zsh — Konsole"))
        .expect("FATAL ERROR");
}
